    ...
    while not crashed:
        if gameState=='game':
            ...
            for event in pygame.event.get():
                ...
                if repr(event)=='''<Event(2-KeyDown {'unicode': 'i', 'key': 105, 'mod': 0, 'scancode': 34})>''':
                    slowTime=True
                    print('slowtime begin')
            ...
            fps =  clock.get_fps()
            pygame.display.set_caption('15-112 Term Project (Version: Public Version) [PVP] FPS:'+str(int(fps)))
            ...
            playerorenemyGravity(enemyGroup,groundGroupHITBOX)
            ...
            jumpPlayer(enemy,key)
            jumpPlayer(player,key)
            ## jump
            
            rollcrawlPlayerOrEnemy(enemy,key,groundGroupHITBOX,displayWidth,turn)
            rollcrawlPlayerOrEnemy(player,key,groundGroupHITBOX,displayWidth,turn)
            ## crawl or roll
            
            if key[enemy.move[4]] and enemy.state=='throwAttack':
                moving=False
                for i in range(2):
                    if key[enemy.move[i]]:
                        moving=True
                        enemy.throwAttack(0.5,i)
                        if key[enemy.move[2]]:
                            enemy.throw()
                            enemyEstoc.master=None
                            enemyEstoc.direction=enemy.direction
                        enemy.changeMask()
                        enemyHitWall = pygame.sprite.spritecollide(enemy, groundGroupHITBOX, False, pygame.sprite.collide_mask)
                        if enemyHitWall:
                            enemy.rect.x -= enemy.dx * [-1, 1][i]
                ...
                if moving==False:
                    if key[enemy.move[2]]:
                        ...
                        enemyEstoc.master=None
                        enemyEstoc.direction=enemy.direction
                    enemy.throwAttack(0.5,None)
            elif enemy.onSky==False and enemy.state=='throwAttack':
                enemy.state='stand'
            # enemy
